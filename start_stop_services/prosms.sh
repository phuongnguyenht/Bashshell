...
#/* -.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.--.-.-.--.-.-.-
#* File Name : auto_start_sers.sh
#* Purpose : auto start services when server starting
...
###########################################################
# Variables Section (Configuration)
###########################################################

thumucchay="/opt/catchupv2"
thamsogrep="catchup_v2.jar"
thumuclog="/opt/catchupv2/log"
...
stop(){
    /usr/bin/ps -ef |grep $thamsogrep |grep -v grep |awk '{print$2}' |xargs kill >/dev/null 2>&1 &
...
kill(){
/usr/bin/ps -ef |grep $thamsogrep |grep -v grep |awk '{print$2}' |xargs kill -9 >/dev/null 2>&1 &
...
case "$1" in
  ...
  stop)
   ...
   ;;
  status)
    t3=`ps -ef | grep $thamsogrep| grep -v grep|wc -l`
    if [ $t3 -eq 1 ]; then
